...
// 2023-12-09 Tweaked from https://www.thingiverse.com/thing:16627 
// by John Knight to create a pulley for my Moonlyte focuser
...
teeth = 56;			// Number of teeth, standard Mendel T5 belt = 8, gives Outside Diameter of 11.88mm
...
motor_shaft = 13.25;	// NEMA17 motor shaft exact diameter = 5
...
module pulley_base(){
    if ( pulley_b_ht < 2 ) { 
        ...
        // shaft (base) of pulley
        /* Wait, what???? - changing to use cylinder!
        rotate_extrude($fn=pulley_b_dia*4)
        {
       ...
       */ 
       cylinder(d=pulley_b_dia, h= pulley_b_ht-idler_ht, $fn = 120);
       cylinder(d=pulley_b_dia+4, h= 2, $fn = 120); 
    }
    
    //label();
}

module pulley_teeth(tooth_distance_from_centre, tooth_width_scale, tooth_depth_scale, pulley_OD){
...
//	translate ([0,0, pulley_b_ht + pulley_t_ht + retainer_ht ]) rotate ([0,180,0])
	...
	difference()
	 {	 
		...
		{
            pulley_base();
            pulley_teeth(tooth_distance_from_centre, tooth_width_scale, tooth_depth_scale, pulley_OD);
		...
		if ( retainer > 0 ) {
            translate ([0,0, pulley_b_ht + pulley_t_ht ]){ 
                cylinder(h=retainer_ht, d1=pulley_OD, d2=pulley_OD + (retainer_ht*2), $fn=90);
                translate([0,0,1.5]){
                    cylinder(h=1, d=pulley_OD + (retainer_ht*2), $fn=90);
                }
            }
        }
		...
		if ( idler > 0 ) {
            ...
            { 
                cylinder(h=idler_ht, d1=pulley_OD + (idler_ht*2), d2=pulley_OD, $fn=90);
                                translate([0,0,-1]){
                    cylinder(h=1, d=pulley_OD + (retainer_ht*2), $fn=90);
                }

            }
            translate ([0,0, pulley_b_ht -10.5]) 
            { 
                cylinder(h=8, d1=pulley_b_dia, d2=pulley_OD + (idler_ht*2), $fn=90);
		...
		translate([0,0,-1])cylinder(r=motor_shaft/2,h=pulley_b_ht + pulley_t_ht + retainer_ht + 20,$fn=motor_shaft*8);
...
module include_captive_nut(){
    if ( pulley_b_ht < m3_nut_flats ) { 
        ...
        union()	{
            ...
            //grub screw hole
            rotate([0,0,22.5])cylinder(r=m3_dia/2,h=pulley_b_dia/2+1,$fn=60);
